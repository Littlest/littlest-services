...
Service.prototype._initEvents = function _initEvents() {
  ...
  self._router.on('data', function (message) {
    var handler = self._handlers[String(message[2])];

    if (!handler) {
      // TODO(schoon) - Respond with an error.
      console.error('No handler for %s.', String(message[2]));
      return;
    }

    when(handler(JSON.parse(message[3])))
      .then(function (result) {
        ...
        self._router.write([message[0], message[1], 'null', result]);
      }, function (err) {
        self._router.write([message[0], message[1], JSON.stringify(err || null), new Buffer(0)]);
      });
  });
